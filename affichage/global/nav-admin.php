<div id="side_menu" class="side-menu">
  <ul class="list-menu">
    <li>
      <a class="link-menu" href="/admin/disponibilite">
        <span id="icon-horaire" class="picto-menu"></span>
        <span class="text-menu"><p>Mes disponibilités</p></span>
      </a>
    </li>
    <li>
      <a class="link-menu" href="/admin/mon-profil">
        <span id="icon-compte" class="picto-menu"></span>
        <span class="text-menu"><p>Mon profil</p></span>
      </a>
    </li>
    <hr class="hr-menu">
    <li>
      <a class="link-menu" href="/admin/gestion-facilitateur">
        <span id="icon-groupe" class="picto-menu"></span>
        <span class="text-menu"><p>Les facilitateurs</p></span>
      </a>
    </li>
    <li>
      <a class="link-menu" href="/admin/gestion-reservation">
        <span id="icon-form" class="picto-menu"></span>
        <span class="text-menu"><p>Gestion des services</p></span>
      </a>
    </li>
    <hr class="hr-menu">
    <li>
      <a class="link-menu" href="/admin/gestion-client">
        <span id="icon-rendezvous" class="picto-menu"></span>
        <span class="text-menu"><p>Les clients</p></span>
      </a>
    </li>
    <li>
      <a class="link-menu" href="/admin/consulter-reservation">
        <span id="icon-book" class="picto-menu"></span>
        <span class="text-menu"><p>Les réservations</p></span>
      </a>
    </li>
    <hr class="hr-menu">
    <li>
      <a class="link-menu" href="/login">
        <span id="icon-direction" class="picto-menu"></span>
        <span class="text-menu"><p>Se déconnecter</p></span>
      </a>
    </li>
  </ul>
</div>
